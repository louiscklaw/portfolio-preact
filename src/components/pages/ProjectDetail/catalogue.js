...
export default class ProjectCatalogue extends Component {
  proj_thumbnail_mouseover(e) {
    ...
    var target_backdrop = target_element.parentNode.querySelector(
      ".blackBackdrop"
    );
      ...
      <>
        <li style={style.li}>
          <a href={"/project_detail/" + href} style={style.a}>
            <div
              style={style.project_thumbnail_container}
              className="project_thumbnail"
              onMouseOver={this.proj_thumbnail_mouseover}
              onMouseLeave={this.proj_thumbnail_mouseleave}
            >
              <div style={style.project_thumbnail_background}></div>
  ...
  render() {
    ...
    let test_jsx = Object.keys(project_list).map(k =>
      this.getProjectDetailLink(k, project_list[k][0], project_list[k][2])
    );

    return (
      <div>
        <ul style={style.ul}>{test_jsx}</ul>
      </div>
    );
  }
}
