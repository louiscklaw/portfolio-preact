...
const BUILD_INFO_TEMPLATE_FILEPATH = path.join(SCRIPT_DIR,'build_info.js.template');
const BUILD_INFO_FILEPATH = path.join(SRC_DIR,'build_info.js');

var build_date = [YEAR_ISO,MONTH_TEXT, DAY_ISO].join('-')

var build_info_content = fs.readFileSync(BUILD_INFO_TEMPLATE_FILEPATH,{encoding:'utf-8'})

var output_build_info = build_info_content.replace(`<<BUILD_DATE>>`, build_date).replace(`<<BUILD_VER>>`, '')
